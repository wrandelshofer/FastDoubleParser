...
abstract class AbstractLenientFloatingPointBitsFromString extends AbstractFloatValueParser {
    ...
    private final StringTrie nanTrie;
    private final StringTrie infinityTrie;
    private final CharSet exponentSeparatorChar;
    private final StringTrie exponentSeparatorTrie;
    ...
    public AbstractLenientFloatingPointBitsFromString(NumberFormatSymbols symbols) {
        ...
        this.exponentSeparatorTrie = new StringTrie(symbols.exponentSeparator());
        this.plusSignChar = CharSet.copyOf(symbols.plusSign());
        this.nanTrie = new StringTrie(symbols.nan());
        this.infinityTrie = new StringTrie(symbols.infinity());
        Set<Character> nanOrInfinitySet = new LinkedHashSet<>();
        for (var s : symbols.nan()) {
            nanOrInfinitySet.add(s.charAt(0));
        }
        for (var s : symbols.infinity()) {
            nanOrInfinitySet.add(s.charAt(0));
        }
    ...
    protected CharSequence filterInputString(String str, int startIndex, int endIndex) {
    ...
    private long parseNaNOrInfinity(CharSequence str, int index, int endIndex, boolean isNegative) {
        ...
        if (nanMatch > 0) {
            index = skipWhitespace(str, index + nanMatch, endIndex);
            if (index == endIndex) {
                return nan();
            }
        }
        ...
        if (infinityMatch > 0) {
            index = skipWhitespace(str, index + infinityMatch, endIndex);
            if (index == endIndex) {
