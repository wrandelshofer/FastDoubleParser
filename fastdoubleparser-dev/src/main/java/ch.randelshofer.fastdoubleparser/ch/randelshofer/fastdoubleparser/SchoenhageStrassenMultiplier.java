 ...
 *     This code is based on the fork by robbymckilliam.
 *     </dt>
 *     <dd><a href="https://github.com/robbymckilliam/bigint">github.com</a></dd>
 * </dl>
 */
public class SchoenhageStrassenMultiplier {
    ...
    public static BigInteger multiplySchoenhageStrassen(BigInteger a, BigInteger b, boolean parallel) {
        ...
        int[] aIntArr = reverse(getMagnitude(a));
        int[] bIntArr = reverse(getMagnitude(b));

        int[] cIntArr = multiplySchoenhageStrassen(aIntArr, a.bitLength(), bIntArr, b.bitLength(), parallel);

        BigInteger c = newBigInteger(1, reverse(cIntArr));
        if (signum < 0) {
            c = c.negate();
        }
    ...
    private static int[] multReverse(int[] a, int[] b, boolean parallel) {
        BigInteger aBigInt = newBigInteger(1, reverse(a));
        BigInteger bBigInt = newBigInteger(1, reverse(b));
        return reverse(getMagnitude(multiply(aBigInt, bBigInt, parallel)));
    }
