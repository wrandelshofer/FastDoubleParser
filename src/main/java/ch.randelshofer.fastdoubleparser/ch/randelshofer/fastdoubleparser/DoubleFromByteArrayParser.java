/*
 * @(#)DoubleFromByteArrayParser.java
 ...
 */
public final class DoubleFromByteArrayParser {

    public DoubleFromByteArrayParser() {
    ...
    private NumberFormatException newNumberFormatException(byte[] str, int off, int len) {
        if (len > 1024) {
     ...
     * @throws NumberFormatException if the string can not be parsed
     */
    public double parseDouble(byte[] str) throws NumberFormatException {
        return parseDouble(str, 0, str.length);
    }

    /**
     * Parses a {@code FloatValue} from a {@link byte} array and converts it
     ...
     * @throws NumberFormatException if the string can not be parsed
     */
    public double parseDouble(byte[] str, int off, int len) throws NumberFormatException {
    ...
    private double parseInfinity(byte[] str, int index, int endIndex, boolean negative, int off) {
        if (index + 7 < endIndex
                //    y  t  i  n  i  f  n  I
    ...
    private double parseNaN(byte[] str, int index, int endIndex, int off) {
        if (index + 2 < endIndex
                //   && str.charAt(index) == 'N'
                && str[index + 1] == (byte) 'a'
                && str[index + 2] == (byte) 'N') {
     ...
     */
    private double parseRestOfDecimalFloatLiteral(byte[] str, int index, int startIndex, int endIndex, boolean isNegative, boolean hasLeadingZero) {
        // Parse 'Digits' production
        // ------------
        // Note: a multiplication by a constant is cheaper than an
        //       arbitrary integer multiplication.
        long mantissa = 0;// mantissa is treated as an unsigned long
        int exponent = 0;
     ...
     *  @param str            the input string
     */
    private double parseRestOfDecimalFloatLiteralTheHardWay(byte[] str, int off, int len) {
    ...
    private int skipWhitespace(byte[] str, int startIndex, int endIndex) {
    ...
    private long tryToParseEightDigits(byte[] str, int offset) {
